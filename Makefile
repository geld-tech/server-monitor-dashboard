...
docker-run-deb:
	sudo docker run -i -t -p 8005:8005 --rm ubuntu:xenial /bin/bash -c ' apt clean all && apt update && apt install -y python wget vim ; \
		...
		apt clean all ; \
		apt update ; \
		apt install -y server-monitor-dashboard ; \
		systemctl status server-monitor-dashboard ; \
		systemctl status server-monitor-dashboard-collector ; \
		$$SHELL '
...
docker-run-rpm:
	sudo docker run -i -t -p 8005:8005 --rm centos:7 /bin/bash -c ' yum clean all && yum install -y python wget vim ; \
		...
		yum install -y server-monitor-dashboard ; \
		systemctl status server-monitor-dashboard ; \
		systemctl status server-monitor-dashboard-collector ; \
		$$SHELL '
